      ...
      <div>
        <table className="w-full border-collapse border border-gray-300">
          ...
          <tbody>
            {data?.map((row, index) => (
              <tr key={index}>
                <td>{JSON.parse(row?.customer || null)?.companyname}</td>
                <td>{row?.invoice_number}</td>
                <td>{JSON.parse(row?.products || null)?.[0]?.quantity} ton</td>
                <td>{(row?.subtotal * row?.primary_rate).toFixed(2)} USD</td>
                <td>
                  {JSON.parse(row?.products || null)
                ...
                <td>
                  {(
                    ...
                    JSON.parse(row?.products || null)?.reduce(
                      (total, product) => {
                        return (
                          total +
                          product?.stocklogs?.reduce(
